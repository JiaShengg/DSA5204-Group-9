...
class FeatureMatching:
    ...
    def __call__(self, real_features, fake_features):
        ...
        match_loss = tf.reduce_mean(tf.math.sqrt(tf.square(real_matrix - fake_matrix)))
        return match_loss
...
def plot_order_book_scatter(lob_sample):
    ...
    # Combine all prices and quantities for dynamic limits
    all_prices = np.concatenate([bid_prices, ask_prices])
    all_sizes = np.concatenate([bid_sizes, ask_sizes])

    # Padding for better visibility (5% of range or a small constant if flat)
    price_range = np.ptp(all_prices)  # ptp = max - min
